...
[**🇷 ⬇ Descargar projecto de R de la práctica y los ejercicios resueltos**](projs/unidad3.zip)
...
Por defecto, la función `write.csv()` escribirá el data table con una variable adicional de índice. El parámetro `row.names` permite definir si queremos o no agregar dicha variable adicional al escribir el data table.  
...
```
En este caso, el resultado a la instrucción anterior luce igual al data table opriginal. 
...
8. En este segundo script, utilizando ambos data tables, programe el código correspondiente para resolver lo siguiente:
  
   8.1. Cree un nuevo data table con la población de mujeres de Aguascalientes por edad.
   
   8.2. Cree un nuevo data table con la población de mujeres de Aguascalientes que estudiaron una licenciatura (o equivalente) por edad.
...
Cuando exportamos conjuntos de datos como data table, R lee no es capaz de identificar por defecto valores categóricos. 
```{r}
str(dt_lic)
```

<br>
En R, para poder identificar una varable categórica como tal, deben ser convertidas en fatcores. Esta conversión trae muchos beneficios a la hora de preprocesar y visualizar los datos, e incluso construir modelos estadísticos con ellos.
