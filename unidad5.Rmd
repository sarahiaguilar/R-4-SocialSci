...
💡 Es posible eliminar variables desde nuestro ambiente desde la ventana de `Environment` de RStudio, pero también es posible con la función `rm()`.

<br> 

## 5.1. Cargando paquetes
```{r}
library(data.table)
library(ggplot2)
```
...
```{r}
# censo_iter <- fread("../data/iter_09_cpv2020_csv/iter_09_cpv2020/conjunto_de_datos/conjunto_de_datos_iter_09CSV20.csv")
```
